...
+ (instancetype)soundNamed:(NSString *)name
{
    ...
    if (![path isAbsolutePath])
    {
        if ([[name pathExtension] isEqualToString:@""])
        {
         ...
         NSBundle *resourcesBundle = [NSBundle bundleForClass:[Sound class]];
        path = [resourcesBundle pathForResource:name ofType:@""];
...
- (void)playSound:(id)soundOrName looping:(BOOL)looping fadeIn:(BOOL)fadeIn
{
    ...
    if (self.pitch != 1.0) {
        sound.sound.enableRate = YES;
        sound.sound.rate = self.pitch;
    }
    else {
        sound.sound.enableRate = NO;
